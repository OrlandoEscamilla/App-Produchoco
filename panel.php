...
<!---------------------------------------------------------------------------------------------------------------------------------------------------->
						...
						<?php  
		    			if($permisos == 1){ ?>
							<a href="#"> <button style="width:160px; background-color:#5ca77c; margin-left: 10px; margin-right:10px; height:20px; padding:5px; padding-top:1px; font-size: 12px;" type="button" class="btn btn-success"  aria-expanded="false" data-toggle="modal" data-target="#nueva_conexion">Nueva conexion</span></button></a>
						<?php  
						}
						else{
...
<div class="contenedor_tabla_conversacion">
  ...
  <thead>
    ...
    <?php 
    	...
    	$sql = "SELECT * FROM tareas WHERE id_tarea IN (SELECT tareas_id_tarea FROM usuarios_has_tareas WHERE usuarios_id_usuario = '$id_user')";
    	...
    	while ($tarea = $ejecutar->fetch_assoc()) { 
			...
			<td><a href="panel.php?id_task=<?php echo $id_tareas; ?>"><?php echo $tarea['nombre_tarea']; ?></a></td>
